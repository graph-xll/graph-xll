...
public class HubScoreWG {
  ...
  public HubScoreWG(String basename,int niter,double eps) throws Exception {
  ...
  public static void main(String[] args) throws Exception {
    ...
    PrintStream ps2 = new PrintStream(new File(basename+"_hubScore_WG_"+niter+ "_" + eps+"_timing.txt"));

		System.out.println("Starting " + basename);
		HubScoreWG hub = new HubScoreWG(basename,niter,eps);
    ...
    ps2.println("Time elapsed (sec) for webgraph loading = " + (System.currentTimeMillis() - startTime)/1000.0);
		...
		PrintStream ps = new PrintStream(new File(basename+"_hubScore_WG_"+niter+ "_"+ eps+".txt"));

    long t2 = System.currentTimeMillis();
	  double[] res = hub.scoreCompute();
    ...
    System.out.println("Total number of iterations = " + hub.iteration);
    ps2.println("Total number of iterations = " + hub.iteration);

    System.out.println("The residual euclidean distance = " + hub.residual);
    ps2.println("The redisual euclidean distance = " + hub.residual);

    System.out.println("The eigen value = " + hub.lambda);
    ps2.println("The eigen value = " + hub.lambda);

    long t4 = System.currentTimeMillis();
    System.out.println("Sorting... be patient");
    Runtime.getRuntime().exec("sort -g -r -k2,2 -k1,1n "+basename+"_hubScore_WG_"+niter+"_"+eps+".txt -o "+basename+"_hubScore_WG_"+niter+"_"+eps+".txt").waitFor();
